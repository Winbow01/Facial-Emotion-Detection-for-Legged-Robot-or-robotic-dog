...
cap = cv2.VideoCapture("/home/pi/ProAPP567/PyPro/happy.mp4")
while True:
    # Find haar cascade to draw bounding box around face    # for  video file
#     ret, frame = cap.read()
...
#     gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)   # for  video file
...
#         cv2.rectangle(color_image, (x, y-50), (x+w, y+h+10), (0, 255, 0), 4)    # for  video file
...
def Comm2Go1( emo_index ):
    if emo_index <0 or emo_index > 6 or   type(emo_index) != int:
        return -1
    ##state: positive 1, negative 0, nutrual 2
    state = -1
    if emo_index == 0 or emo_index == 1 or emo_index == 2 or emo_index == 5 :
        state = 0
    elif emo_index == 3 or emo_index == 6 :
        state = 1
    elif emo_index == 4 :
        state = 2

    ## subprocess.call(["cd unitree_legged_sdk-3.8.0/build/","-a"])# 数组作为参数运行命令
